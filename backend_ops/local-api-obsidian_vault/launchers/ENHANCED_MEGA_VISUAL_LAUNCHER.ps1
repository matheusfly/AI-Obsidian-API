# ENHANCED_MEGA_VISUAL_LAUNCHER.ps1
# Enhanced Visual System with Real MCP Integration and JSON Crack

Write-Host "🚀 ENHANCED MEGA VISUAL SYSTEM LAUNCHER" -ForegroundColor Cyan
Write-Host "🔍 Integrating MCP tools and JSON Crack for real data..." -ForegroundColor Yellow
...
# Function to create enhanced mega visual main file with real MCP integration
function Create-EnhancedMegaVisualMain {
    Write-Host "🔧 Creating enhanced mega visual main file with MCP integration..." -ForegroundColor Yellow
...
async def crawl_real_data():
    ...
    try:
        ...
        results["crawl_results"][servicesservices/n8n_workflows"] = n8n_workflows
        results["summary"][servicesservices/n8n_workflows_count"] = len(n8n_workflows)
...
async def discover_real_endpoints():
    ...
    try:
        ...
        fastapi_files = glob.glob(servicesservices/vault-api/*.py")
...
async def discover_real_mcp_tools():
    ...
    try:
        ...
        mcp_config_files = [
            ...
            ".cursor/mcp.json",
            config//mcp.json",
            servicesservices/vault-api/mcp.json"
        ]
...
async def discover_n8n_workflows():
    ...
    try:
        ...
        n8n_files = glob.glob(servicesservices/n8n-workflows/*.json")
        n8n_files.extend(glob.glob(servicesservices/n8n/**/*.json", recursive=True))
...
def generate_endpoint_cards(endpoints):
    ...
    for endpoint in endpoints:
        ...
        <div class="node-card">
            ...
            <div class="node-meta">
                <span>File: {endpoint.get(scripts/le', 'unknown')}</span>
...
async def mcp_debug():
    ""scripts/ MCP integration and show real data"""
    ...
    <body>
        <div class="container">
            ...
            <div class=scripts/-section">
                <div class=scripts/-title">🔍 MCP Tools Discovered</div>
...
{json.dumps(real_mcp_tools, indent=2)}
            ...
            <div class=scripts/-section">
                <div class=scripts/-title">🔧 MCP Tools List</div>
                {generate_mcp_tools_html(real_mcp_tools)}
            </div>
            
            <div class=scripts/-section">
                <div class=scripts/-title">📊 System Status</div>
                <div class=scripts/-content">
...
def generate_mcp_tools_html(tools):
    ...
    if not tools:
        return '<div class=scripts/-content">No MCP tools found</div>'
...
'@
    ...
    $enhancedMain | Out-File -FilePath servicesservices/vault-api/main_enhanced_mega_visual.py" -Encoding UTF8
    Write-Host "  ✅ Created enhanced mega visual main file" -ForegroundColor Green
}
...
# Create enhanced mega visual main file
Create-EnhancedMegaVisualMain
...
# Start the service
Write-Host "`n🚀 STARTING ENHANCED MEGA VISUAL SYSTEM..." -ForegroundColor Green
Write-Host "  📡 Port: $availablePort" -ForegroundColor Gray
Write-Host "  🔧 Mode: Real MCP Integration + JSON Crack + Complete Data Fetching" -ForegroundColor Gray

# Start the service in background
$process = Start-Process -FilePath "python" -ArgumentList "-m", "uvicorn", "main_enhanced_mega_visual:app", "--host", "0.0.0.0", "--port", $availablePort, "--reload" -WorkingDirectory servicesservices/vault-api" -PassThru -WindowStyle Hidden
...
if ($serviceReady) {
    Write-Host "`n🎉 ENHANCED MEGA VISUAL SYSTEM LAUNCHED SUCCESSFULLY!" -ForegroundColor Green
    Write-Host "================================================================" -ForegroundColor Cyan
    
    Write-Host "`n🌐 ACCESS POINTS:" -ForegroundColor Cyan
    Write-Host "🚀 Main Dashboard:            $serviceUrl" -ForegroundColor White
    Write-Host "📡 Real API Data:             $serviceUrl/mega/real-api-nodes" -ForegroundColor White
    Write-Host "🔧 Real MCP Tools:            $serviceUrl/mega/real-mcp-nodes" -ForegroundColor White
    Write-Host "🕷️ Crawl Real Data:           $serviceUrl/mega/crawl-real-data" -ForegroundColor White
    Write-Host "🐛 MCP Debug:                 $serviceUrl/mega/mcp-debug" -ForegroundColor White
    Write-Host "📊 JSON Crack Viewer:         $serviceUrl/mega/jsoncrack-viewer" -ForegroundColor White
    ...
    Write-Host "`n✨ Your ENHANCED MEGA VISUAL SYSTEM is ready!" -ForegroundColor Cyan
    Write-Host "🎨 Real MCP integration with actual data discovery!" -ForegroundColor Yellow
    Write-Host "🕷️ Automatic crawling of your real system files!" -ForegroundColor Yellow
    
} else {
    Write-Host "`n❌ SERVICE FAILED TO START" -ForegroundColor Red
    Write-Host "  🔧 Try running manually:" -ForegroundColor Yellow
    Write-Host "  cd vault-api && python main_enhanced_mega_visual.py" -ForegroundColor Gray
}

Write-Host "`n💡 PRO TIPS:" -ForegroundColor Yellow
Write-Host "• This system discovers REAL data from your actual files" -ForegroundColor White
Write-Host "• MCP integration shows your actual MCP tools and configurations" -ForegroundColor White
Write-Host "• Real-time crawling of API endpoints, Docker services, and workflows" -ForegroundColor White
Write-Host "• Professional visual nodes with actual system data" -ForegroundColor White
Write-Host "• Debug tools to troubleshoot MCP integration issues" -ForegroundColor White
